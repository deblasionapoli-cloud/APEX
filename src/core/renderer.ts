...
export function renderFrame(state: State): string {
  ...
  if (
    state.display_speech.length > 0 &&
    state.display_speech !== state.last_speech &&
    animation_phase % 4 < 2
  ) {
    mouth = '|    {     ---     }    |';
  }
  ...
  if (entropy > 50 || state.speech_sentiment === 'chaotic') {
    finalSpeech = finalSpeech
      .split('')
      .map((c) =>
        Math.random() * 100 < entropy * 0.1
          ? ['#', '!', '_', '/', 'X'][Math.floor(Math.random() * 5)]
          : c
      )
      .join('');
  }
  ...
  const generateLog = (phaseIdx: number, side: 'L' | 'R') => {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      if (entropy > 40 && Math.random() < entropy / 100) {
        entry = Math.floor(Math.random() * 65535)
          .toString(16)
          .toUpperCase()
          .padStart(6, '0');
      }
  ...
  let frame = ['', ...mergedLines, '', `[ ${finalSpeech} ]`, ''].join('\n');
